...
if ( isset( $_POST['scos_ntfy_test'] ) && check_admin_referer( 'scos_ntfy_test' ) && $ntfy_ok ) {
	...
	$result = $ntfy_client->send(
		...
		[
			...
			'click'    => admin_url( 'admin.php?page=site-essentials-essentials&tab=performance' ),
		]
	);
...
<div class="scos-card">
	...
	<div class="scos-card__body">
		...
		<?php if ( ! $ntfy_loaded ) : ?>
			<p class="description"><?php esc_html_e( 'Monitoring class not found. Ensure brighter-core is active.', 'site-essentials' ); ?></p>
...
define('NTFY_PASSWORD', 'your-password');</pre>
		...
		<?php elseif ( ! $ntfy_ok ) : ?>
			<p><strong><?php esc_html_e( '⚠ ntfy is enabled but not fully configured.', 'site-essentials' ); ?></strong></p>
...
<div class="scos-card">
	...
	<div class="scos-card__body">
						...
						<?php elseif ( $is_on ) : ?>
							<span>
								...
								<?php if ( $mon['status'] === 'partial' ) : ?>
									<span class="description"> (<?php esc_html_e( 'partial', 'site-essentials' ); ?>)</span>
								<?php endif; ?>
							</span>
						<?php else : ?>
							<span>
		...
		<p class="description">
			...
			<?php esc_html_e( 'constants in wp-config.php (e.g. NTFY_MONITOR_SMTP). If a constant is omitted, brighter-core falls back to enabled for most monitors; forms and cron stay opt-in (off).', 'site-essentials' ); ?>
		</p>
